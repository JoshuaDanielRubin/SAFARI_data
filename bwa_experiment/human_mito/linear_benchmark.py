...
# Function to calculate percentage difference for incorrectly mapped reads
def calculate_percentage_diff_negative(group):
    safari_values = group.loc[group.index.get_level_values('Aligner') == 'safari']
    return safari_values.values - group  # Safari values - Tool values to get negative differences
...
# Group by Sampling Rate and apply the function for incorrectly mapped reads
df_diff_negative = df.groupby('Sampling Rate').apply(calculate_percentage_diff_negative)
df_diff_negative = df_diff_negative[df_diff_negative.index.get_level_values('Aligner') != 'safari'].reset_index()
...
# Now create subplots for incorrectly mapped reads
# Determine the number of rows required for a 2-column layout
num_rows = (len(unique_sampling_rates) + 1) // 2
...
# Iterate through each sampling rate and create a bar plot for Incorrect (%)
for i, rate in enumerate(unique_sampling_rates):
    df_subset = df_diff_negative[df_diff_negative['Sampling Rate'] == rate]
    sns.barplot(data=df_subset, x='Aligner', y='Incorrect (%)', ax=axs[i])
...
plt.savefig("incorrect_mapping_benchmark.png")
plt.close()
