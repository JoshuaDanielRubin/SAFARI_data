...
subsample_bam() {
  ...
  out_dir="subsampled_reps_corr/${target_depth}x"
...
for bam_file in ${bam_dir}/*.bam; do
  ...
  #for replicate in {1..5}; do
  #  nice -19 parallel -j 50 subsample_bam {} $chr 1 $replicate ::: $bam_file
  #done

  # Subsample at target depth of 2x
   #for replicate in {1..5}; do
   #  nice -19 parallel -j 55 subsample_bam {} $chr 2 $replicate ::: $bam_file
   #done
