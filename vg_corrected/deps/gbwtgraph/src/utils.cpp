...
char convertBaseToRymer(const char base) {
    switch (base) {
        ...
        default:
            ...
            std::cerr << "Invalid character: " << base << std::endl;
            return '\0';
    }
}
...
std::string
reverse_complement(const std::string& seq, bool rymer)
{
  std::string result = seq;
  reverse_complement_in_place(result, rymer);
  return result;
}

void
reverse_complement_in_place(std::string& seq, bool rymer)
{
