/** \file rymer_main.cpp
 *
 * Defines the "vg rymer" subcommand, which builds the rymer index.
 *
 ...
 *
 * The index contains either all or haplotype-consistent rymers. Indexing all
 * rymers from complex graph regions can take a long time (e.g. tens of hours
 * vs 5-10 minutes for 1000GP), because many windows have the same rymer.
 * As the total number of rymers is manageable (e.g. 1.5x more for 1000GP)
 * it should be possible to develop a better algorithm for finding the rymers.
 *
...
void help_rymer(char** argv) {
    ...
    std::cerr << "    -k, --kmer-length N     length of the kmers in the index (default " << IndexingParameters::rymer_k << ", max " << gbwtgraph::DefaultRymerIndex::key_type::KMER_MAX_LENGTH << ")" << std::endl;
    std::cerr << "    -w, --window-length N   choose the rymer from a window of N kmers (default " << IndexingParameters::rymer_w << ")" << std::endl;
    std::cerr << "    -c, --closed-syncmers   index closed syncmers instead of rymers" << std::endl;
    std::cerr << "    -s, --smer-length N     use smers of length N in closed syncmers (default " << IndexingParameters::rymer_s << ")" << std::endl;
    std::cerr << std::endl;
...
int main_rymer(int argc, char** argv) {
    ...
    while (true) {
        ...
        case 'c':
            ...
            break;
        case 's':
            IndexingParameters::rymer_s = parse<size_t>(optarg);
            break;
        case 'd':
    ...
    // rymer index.
    std::unique_ptr<gbwtgraph::DefaultRymerIndex> index;
    if (load_index.empty()) {
        index = std::make_unique<gbwtgraph::DefaultRymerIndex>(IndexingParameters::rymer_k, 
            (use_syncmers ? IndexingParameters::rymer_s : IndexingParameters::rymer_w),
            use_syncmers);
    } else {
        if (progress) {
            std::cerr << "Loading rymerIndex from " << load_index << std::endl;
        }
        index = vg::io::VPKG::load_one<gbwtgraph::DefaultRymerIndex>(load_index);
    }
    ...
    // Build the index.
    if (progress) {
        std::cerr << "Building rymerIndex with k = " << index->k();
    ...
    if (progress) {
        ...
        std::cerr << "rymer occurrences: " << index->values() << std::endl;
...
// Register subcommand
static vg::subcommand::Subcommand vg_rymer("rymer", "build a rymer index or a syncmer index", vg::subcommand::TOOLKIT, main_rymer);
